...
def recv_packet(sock) -> bytes:
    ...
    # lLer os primeiros 4 bytes para saber o tamanho
    header = sock.recv(4)
    if not header:
        return None
    ...
    # Ler o conteúdo até completar o tamanho avisado
    data = b''
...
def create_key_exchange_packet(sender: str, receiver: str, public_key: bytes) -> bytes:
    ...
    packet = {
        ...
        "sender": sender,
        "receiver": receiver,
        # converter para string para o JSON aceitar
        "public_key": public_key.decode('utf-8')
    }
    return json.dumps(packet).encode('utf-8')

def create_exit_packet(sender: str, receiver: str) -> bytes:
    """Avisa o outro lado que a conexão foi encerrada."""
    packet = {
        "type": "EXIT",
        "sender": sender,
        "receiver": receiver
